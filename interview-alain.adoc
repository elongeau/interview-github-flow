...
Il faut prendre le problème par l'angle de l'object final, l'idée c'est de pouvoir dire à un utilisateur métier qu'on va avoir le minimum de temps possible pour passer d'une idée à une mise en production et à donner la fonctionnalité à l'utilisateur.
...
De toute façon ils seront toujours moins importants quand on a l'habitude de le faire très fréquemment plutôt que quand on le fait une fois par mois. Plus on le fait plus c'est sécurisé.
...
Après ça ne fait qu'un framework de plus parmi les 250 qu'on utilise déjà donc ce n'est pas un gros risque. Il faut évidemment les maitriser un minimum, ça va rajouter sans doute un petit peu de travail de maintenabilité et de maintenance.
